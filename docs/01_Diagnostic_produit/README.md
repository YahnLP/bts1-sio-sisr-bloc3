# 01 – Objectifs et ressources

Cette séance introductive a pour objectif d'installer les bases du **diagnostic scientifique d'un produit cosmétique** et du vocabulaire associé. 

Les notions abordées (corps pur, mélange homogène/hétérogène, lecture INCI) seront réinvesties tout au long de l'année dans des situations professionnelles liées à la cosmétologie et à l'épreuve E2.

---

## 🎯 Objectifs

### Objectifs scientifiques

* Distinguer **corps pur** et **mélange**
* Distinguer **mélange homogène** et **mélange hétérogène**
* Définir la notion de **phase**
* Lire et exploiter une **liste INCI**

### Objectifs méthodologiques (E2)

* Lire une situation professionnelle et un dossier technique
* Extraire les **informations pertinentes** d'un document
* Structurer une **réponse justifiée** avec un vocabulaire scientifique

---

## 🧠 Compétences travaillées

* **Analyser** : extraire des informations d'un dossier technique (liste INCI)
* **Communiquer** : utiliser un vocabulaire scientifique précis
* **Argumenter** : justifier une classification (corps pur / mélange / homogène / hétérogène)

---

## 📥 Ressources à télécharger

### 📘 Documents élève

➡️ [Télécharger la fiche élève (PDF)](01a_fiche_eleve.pdf)

➡️ [Télécharger la trace écrite (PDF)](01b_trace_ecrite.pdf)

➡️ [Télécharger l'évaluation formative (PDF)](01c_evaluation.pdf)

### 🔧 Fiche méthode associée

➡️ [Fiche méthode 01 – Justifier une réponse en physique-chimie](../Methodologie/01_fiche_methode/)

---

## 🔗 Navigation

⬅️ Séance précédente : *Aucune (première séance)*

➡️ Séance suivante : [S02 – Solutions : solvant, soluté, dissolution](../02_Solutions/)
